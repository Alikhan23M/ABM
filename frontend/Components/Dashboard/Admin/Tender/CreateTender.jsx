...
const CreateTender = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await AxiosRequest.post('/api/tender/createTender', formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      toast.success('Tender created successfully!');
      console.log('Created Tender:', response.data); // Optionally handle the response data
      ...
      toast.error(err.response ? err.response.data.message : 'Failed to create tender');
    }
  };
        ...
        <Card className="p-8 shadow-lg rounded-lg bg-white">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6 border rounded-lg p-2 border-black">
              <CloudinaryUpload onUploadSuccess={handleImageUploadSuccess} folder="Tenders" isSubmitted={isSubmitted} setIsSubmitted={setIsSubmitted} />
            </div>
            {formData.imageUrl && (
